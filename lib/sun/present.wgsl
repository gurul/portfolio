...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  // SITE PATCH (final view only; debug views above are untouched): the canvas
  // contributes light and nothing else. The example lit a gray albedo floor
  // and drew the emitter disc on top — on the page that floor read as a
  // visible square behind the sun, and the disc competed with the site's own
  // sun glyph. Here the irradiance field itself is the output (lighting.x is
  // its gain), the emitter is never drawn (the CSS glyph is the sun), and a
  // radial falloff takes the glow to exactly zero before the canvas edge.
  // Output is premultiplied alpha (alpha = brightest channel, so rgb <=
  ...
  let alpha = max(glow.r, max(glow.g, glow.b));
  return vec4f(glow, alpha);
}
