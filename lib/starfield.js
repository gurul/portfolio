...
// One star per grid cell, placed in the cell's middle 60% so its halo never
// crosses into a neighbor. Brightness, size, twinkle rate and phase all hang
...
fn star_layer(pixel: vec2f, cell: f32, seed: f32, drift: vec2f, t: f32) -> f32 {
  ...
  let d = distance(p, star_pos);
  let size = mix(0.6, 1.5, fract(b * 9.0));
  let core = exp(-(d * d) / (2.0 * size * size));
  let halo = exp(-d / (7.0 * size)) * 0.08;
  let rate = mix(0.25, 1.7, fract(b * 5.0));
  let twinkle = 0.62 + 0.38 * sin(t * rate + b * 61.0);
  return (core + halo) * twinkle * mix(0.18, 1.0, b * b);
}
